...
# 📌 1️⃣ Train EPC Cost Estimation Model
def train_epc_model():
    print("Training EPC Cost Model...")

    # Dummy data: [Project Size (MW), Material Cost (₹ Crore), Labor Cost (₹ Crore), Equipment Cost (₹ Crore)]
    data = pd.DataFrame({
    ...
    print(f"EPC Cost Model Trained! MAE: {error:.2f}")
    joblib.dump(model, "models/epc_cost_model.pkl")

# 📌 2️⃣ Train Smart Grid Load Forecasting Model
def train_smart_grid_model():
    ...
    print(f"Smart Grid Model Trained! MAE: {error:.2f}")
    joblib.dump(model, "models/grid_forecasting.pkl")

# 📌 3️⃣ Train Procurement Optimization Model
def train_procurement_model():
    ...
    print(f"Procurement Model Trained! MAE: {error:.2f}")
    joblib.dump(model, "models/procurement_ai.pkl")

# 📌 4️⃣ Train Predictive Maintenance Model
def train_maintenance_model():
    ...
    print(f"Predictive Maintenance Model Trained! MAE: {error:.2f}")
    joblib.dump(model, "models/predictive_maintenance.pkl")

# 📌 5️⃣ Train Oil & Gas Monitoring Model
def train_oil_gas_model():
    ...
    print(f"Oil & Gas Monitoring Model Trained! MAE: {error:.2f}")
